/*  Copyright 240 Timothy Gedney
 *
 *  Expanded upon the provided methods in the .h file
 */

#include <inc/gradebook.h>
#include <cstddef>
//  using size_t
#include <vector>
using std::vector;
#include <cmath>

//  adds new grade to the vector using push_back method
void GradeBook::Add(char grade) {
  grades.push_back(grade);
}

//  returns grade at supplied index
char GradeBook::operator[](size_t index) const {
  if (index >= grades.size()) {
    return '\0';
  }
  return grades[index];
}

//  adds two gradebook instnaces together
const GradeBook GradeBook::operator+(const GradeBook& that) const {
  GradeBook addGrades;

  for (size_t i = 0; i < grades.size(); i++) {
    addGrades.Add(grades[i]);
  }
  for (size_t i = 0; i < that.grades.size(); i++) {
    addGrades.Add(that[i]);
  }
  return addGrades;
}

//  returns true when two gradebooks have the same average grade value
bool GradeBook::operator==(const GradeBook& that) const {
  GradeBook grade = CreateGradeBook();
  if ((that.GetSize() == 0 && grade.GetSize() == 0) ||
  that.GetGPA() == grade.GetGPA()) {
    return true;
  } else {
    return false;
  }
}

//  returns true when two gradebooks have the same average grade value
bool GradeBook::operator!=(const GradeBook& that) const {
  return !operator==(that);
}

/*  returns true when the calling instance has a smaller grade average than the parameter's average
 *  if they are equal, it compares their max grades
 */
bool GradeBook::operator<(const GradeBook& that) const {
  GradeBook grade = CreateGradeBook();
  if (that.GetSize() == 0 || that.GetGPA() < grade.GetGPA()) {
    return true;
  } else if (that.GetGPA() == grade.GetGPA()) {
    if (that.GetMax() < grade.GetMax()) {
      return true;
    } else {
      return false;
    }
  } else {
    return false;
  }
}

/*  returns the GPA of a gradebook, if/else statements convert the letters to numerical values &
 *  calculates the GPA then converts the GPA back into a letter grade to be returned
 */
char GradeBook::GetGPA() const {
  if (grades.size() == 0) {
    return -1;
  }
  double sum = 0;
  for (size_t i = 0; i < grades.size(); i++) {
    if (grades[i] == 'A') {
      sum += 4;
    } else if (grades[i] == 'B') {
      sum += 3;
    } else if (grades[i] == 'C') {
      sum += 2;
    } else {
      sum += 1;
    }
  }
  double gpa = sum / grades.size();
  char answer;
  if (gpa >= 3.5) {
    answer = 'A';
  } else if (gpa >= 2.5) {
    answer = 'B';
  } else if (gpa >= 1.5) {
    answer = 'C';
  } else if (gpa >= 0.5) {
    answer = 'D';
  } else {
    answer = 'F';
  }
  return answer;
}

//  finds the highest grade in a gradebook by using if/else statements
char GradeBook::GetMax() const {
  if (grades.size() == 0) {
    return '\0';
  }
  char max = 'F';
  char current;
  for (size_t i = 0; i < grades.size(); i++) {
    current = grades[i];
    if (current == 'A') {
      max = current;
    } else if (current == 'B' && max != 'A') {
      max = current;
    } else if (current == 'C' && (max != 'A' || max != 'B')) {
      max = current;
    } else if (current == 'D' && (max != 'A' || max != 'B' || max != 'C')) {
      max = current;
    }
  }
  return max;
}

//  returns the size of the grades vector
const size_t GradeBook::GetSize() const {
  return grades.size();
}

//  creates a new gradebook instance from the vector to be compared in methods
GradeBook GradeBook::CreateGradeBook() const {
  GradeBook gradeBook;
  for (size_t i = 0; i < grades.size(); i++) {
    gradeBook.Add(grades[i]);
  }
  return gradeBook;
}